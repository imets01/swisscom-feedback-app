...
export default function AdminDashboard() {
  ...
  // Check if interview_type_count exists in stats, and safely access it
  const interviewTypeCount = stats?.interview_type_count || {};
  if (!interviewTypeCount || Object.keys(interviewTypeCount).length === 0) {
    return <div>No interview type data available.</div>;
  }

  // Sort the interview types based on the count in descending order
  const sortedInterviewTypes = Object.entries(interviewTypeCount).sort(
    (a, b) => b[1] - a[1]
  );
