...
export default function AdminDashboard() {
  ...
  // Prepare the data for the Bar chart (using interview type counts)
  const interviewTypes = stats.interview_type_distribution || {};
  const interviewTypeLabels = Object.keys(interviewTypes);
  const interviewTypeValues = Object.values(interviewTypes);

  const barChartData = {
    labels: interviewTypeLabels, // Categories for the X-axis
    datasets: [
      {
        label: "Interview Type Count",
        data: interviewTypeValues, // Values for each category
